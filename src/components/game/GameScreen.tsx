...
export function GameScreen({ config, progress, onPause, onCharacterComplete }: GameScreenProps) {
  ...
  // 文字ハイライト用
  const characterHighlightAnim = useRef(new Animated.Value(1)).current;
  ...
  const recordingTimerRef = useRef<NodeJS.Timeout | null>(null); // 録音タイマー参照
  ...
  const forceInitTimerRef = useRef<NodeJS.Timeout | null>(null); // 強制初期化タイマー参照
  ...
  // セットの開始時に使用済み文字をリセット
  const resetUsedCharacters = () => {
    console.log('GameScreen: セット内の使用済み文字をリセットします');
    usedCharactersInSet.current = [];
  };
  ...
  const selectNewCharacter = () => {
    try {
      ...
      // セット内で使用可能な文字を取得（未収集かつセット内未使用）
      const availableChars = getAvailableCharacters()
        .filter((char) => char !== prevChar && !usedCharactersInSet.current.includes(char));
      ...
      if (availableChars.length > 0) {
        ...
        // 使用済みリストに追加
        usedCharactersInSet.current.push(newChar);
        ...
        // セット内で未使用の全文字から選択
        const allUnusedChars = config.characters
          .filter((char) => char !== prevChar && !usedCharactersInSet.current.includes(char));

        if (allUnusedChars.length > 0) {
          const randomIndex = Math.floor(Math.random() * allUnusedChars.length);
          const newChar = allUnusedChars[randomIndex];
          ...
          // セット内のすべての文字を使い切った場合は使用済みリストをリセット
          console.log('GameScreen: セット内の全文字を使用しました。リセットします。');
          usedCharactersInSet.current = [];
          
          // 再度選択を試みる
          const freshChars = config.characters.filter((char) => char !== prevChar);
          if (freshChars.length > 0) {
            const randomIndex = Math.floor(Math.random() * freshChars.length);
            const newChar = freshChars[randomIndex];
  ...
  const startRecordingProcess = async () => {
    ...
    try {
      ...
      // オーディオモードを設定（共通設定を使用）
      await Audio.setAudioModeAsync(AUDIO_MODE_CONFIG.RECORDING);
      ...
      // ステージモードに応じた制限時間後に自動停止するタイマーを設定
      const timeLimitMs = stageMode.timeLimit || RECORDING_TIMER_CONFIG.GAME_DEFAULT_DURATION_MS;
      recordingTimerRef.current = setTimeout(() => {
        console.log(`GameScreen: ${timeLimitMs / 1000}秒タイマー発動`);
        ...
        if (recording.current) {
          console.log('GameScreen: 自動停止を実行します');
          handleStopRecording();
        } else {
          console.log('GameScreen: 録音オブジェクトが存在しないため自動停止をスキップ');
        }
      }, timeLimitMs);
  ...
  // タイマー処理
  useEffect(() => {
    // 初期化が完了し、ゲームが開始されたらタイマーを開始
    if (!isInitializing && !showPause && !isSessionComplete && !showStartButton) {
  ...
  }, [isInitializing, showPause, isSessionComplete, showStartButton]);
  ...
  useEffect(() => {
    const handlePauseChange = async () => {
      if (showPause) {
        ...
        // 開始ボタンが押されていない場合はスキップ
        if (showStartButton || !isFirstRecordingStarted.current) {
          console.log('GameScreen: まだゲームが開始されていないため、ポーズ解除後の録音開始をスキップします');
          return;
        }
  ...
  useEffect(() => {
    ...
    forceInitTimerRef.current = setTimeout(async () => {
      try {
        ...
        if (isInitializing) {
          ...
          if (isListeningRef.current) {
            console.log('GameScreen: 録音中のため、強制初期化のクリーンアップをスキップします');
          ...
          setTimeout(() => {
            ...
            // 録音ロックがかかっていない場合かつ録音中でない場合かつスタートボタンが押された場合のみ実行
            if (!isRecordingLocked.current && !isListeningRef.current && !showPause && !showLevelUp && !isProcessing && !showStartButton) {
    ...
    return () => {
      if (forceInitTimerRef.current) {
        ...
        forceInitTimerRef.current = null;
      }
    };
  }, []);
  ...
  const setupInitialCharacter = () => {
    ...
    // 使用済みリストに追加
    usedCharactersInSet.current.push(initialChar);
  ...
  useEffect(() => {
    ...
    const setDefaultCharacter = () => {
      // スタートボタンが押されるまで文字を表示しない
      console.log('GameScreen: スタートボタンが押されるまで文字を表示しません');
      setCurrentCharacter('');
      latestCharacter.current = '';
    };
  ...
  const selectAndStartRecording = async () => {
    ...
    // 音声読み上げ中なら中止
    if (isVoiceReading) {
      console.log('GameScreen: 音声読み上げ中のため、文字選択をスキップします');
      return;
    }
  ...
  const handleCharacterTransition = async (result: 'correct' | 'incorrectA' | 'incorrectB') => {
    ...
    try {
      ...
      // 判定結果表示の待機を短縮
      try {
        await new Promise((resolve) => setTimeout(resolve, 200)); // さらに短縮
      } catch (timerError) {
  ...
  const handleStopRecording = async () => {
    ...
    try {
      ...
      if (recordingTimerRef.current) {
        clearTimeout(recordingTimerRef.current);
        recordingTimerRef.current = null;
        console.log('GameScreen: 録音タイマーをクリアしました');
      }
      ...
      try {
        ...
        if (!isAIReady) {
          // AIモデルが読み込み中の場合、ポップアップを表示
          console.log('GameScreen: AIモデルが読み込み中です');
          ...
          console.log('GameScreen: AIモデルの読み込みが完了しました');
        }
        
        // 発音の評価（AIによる判定）
        // 録音開始時の文字を使用（latestCharacter.currentには録音開始時の文字が保存されている）
        const evaluationResult = await voiceService.evaluateRecording(latestCharacter.current, recordingUri || undefined);
        result = evaluationResult.result;
        aiResult = {
          confidence: evaluationResult.topResults[0]?.confidence || 0,
          top3: evaluationResult.topResults,
        };
        console.log('GameScreen: AI判定結果', evaluationResult);
      } catch (aiError) {
        console.error('GameScreen: AI判定エラー', aiError);
        
        // エラー時は録音を破棄して、ユーザーに再試行を促す
        Alert.alert(
          'ごめんなさい',
          'はつおんの はんてい が できませんでした。\nもういちど ためしてみてください。',
          [
            {
              text: 'OK',
              onPress: () => {
      ...
      if (userId && recordingUri) {
        practiceStorageService
          .savePracticeData({
            ...
            aiResult,
            metadata: {
              stageType: config.type || 'unknown',
              trainingLevel: stageMode.trainingLevel,
              timeLimit: stageMode.timeLimit,
            },
          })
          .catch((error) => {
  ...
  useEffect(() => {
    if (!isInitializing && initialSetupComplete.current && !isFirstRecordingStarted.current) {
      console.log('GameScreen: 初期化完了 - 開始ボタンの押下を待機');
      // 開始ボタンを表示（既にtrueに設定済み）
    }
  }, [isInitializing]);

  // 開始ボタンが押された時の処理
  const handleStartGame = () => {
    console.log('GameScreen: ゲーム開始ボタンが押されました');
  ...
  const handlePause = async () => {
    ...
    if (recordingTimerRef.current) {
      clearTimeout(recordingTimerRef.current);
      recordingTimerRef.current = null;
      console.log('GameScreen: 一時停止 - 録音タイマーをクリアしました');
    }
  ...
  const startInitialization = async () => {
    try {
      ...
      if (forceInitTimerRef.current) {
        clearTimeout(forceInitTimerRef.current);
        forceInitTimerRef.current = null;
        console.log('GameScreen: 強制初期化タイマーをクリアしました');
      }
    } catch (error) {
      console.error('初期化エラー:', error);
      // エラーがあっても初期化は完了とする
      ...
      if (forceInitTimerRef.current) {
        clearTimeout(forceInitTimerRef.current);
        forceInitTimerRef.current = null;
        console.log('GameScreen: 強制初期化タイマーをクリアしました（エラー時）');
      }
    }
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <ImageBackground
        ...
        <View
          ...
          <TouchableOpacity
            ...
            style={{
              width: 120,
              height: 120,
              backgroundColor: isListeningRef.current && !isJudging.current && !isVoiceReading ? '#FF3B30' : '#999', // 録音中は赤、それ以外はグレー
              borderRadius: 60,
              justifyContent: 'center',
              alignItems: 'center',
              shadowColor: '#000',
        ...
        {/* 開始ボタン */}
        {showStartButton && !isInitializing && (
          <View
            style={{
              position: 'absolute',
              top: 0,
          ...
          <View
            ...
            <View
              ...
              <View
                ...
                <MaterialCommunityIcons name="robot" size={60} color="#FF6B6B" />
              </View>
              <Text style={{ fontFamily: 'font-mplus-bold', fontSize: 20, color: '#333', marginBottom: 8 }}>
                AIよみこみちゅう
              </Text>
              <Text style={{ fontFamily: 'font-mplus', fontSize: 14, color: '#666', marginBottom: 16, textAlign: 'center' }}>
                AIモデルを よみこんでいます。{'\n'}しばらく おまちください。
              </Text>
